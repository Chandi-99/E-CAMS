               ...
               int.TryParse(txt_relation.Text, out check_in) || !int.TryParse(txt_year.Text, out check_in) || !int.TryParse(txt_contact.Text, out check_in))
                ...
                else
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        cmd.CommandText = "insert into old_boy_has_sport (old_boy_old_boy_id,sport_sport_id,best_relation,best_relation_year,contact) values('" + Convert.ToInt32(dt.Rows[old_id - 1]["old_boy_id"].ToString()) + "','" + spo_id + "','" + txt_relation.Text + "','" + txt_year.Text + "'" + ",'" + txt_contact.Text + "')";
                        cmd.ExecuteNonQuery();
